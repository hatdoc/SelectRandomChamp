...
const MISSION_DATA = {
  ...
  'targets': {
    'ko_KR': [
      "영어 한 마디도 안 하기", "영어만 사용하기 (교포 방송)", "죽을 때마다 애교/벌칙 수행", "20분 안에 게임 끝내기", "성공 시 시청자에게 치킨", 
      "한 손으로만 게임하기", "성공 시 노래 한 곡 완창", "실패 시 다음 판 서포터", "성공 시 룰렛 10번 돌리기", "반말로 방송하기", 
      "존댓말로만 채팅치기", "30초마다 '역시 이상호' 외치기", "물 머금고 게임하기", "사투리만 쓰기", "스쿼트 20개 하면서 하기",
      "마우스 감도 2배로 하기", "성공 시 시청자가 정해준 닉네임 변경", "실패 시 다음 판 노스킨"
    ],
    'en_US': [
      "No English Challenge", "English Only Challenge", "Penalty on every death", "Finish game before 20m", "Giveaway on success", 
      "One-hand play challenge", "Sing a song on success", "If failed, play Supp next", "Spin roulette 10x on success", "Talk in a funny accent", 
      "Shout 'YEOKSI SANGHO' every 30s", "Water in mouth challenge", "Play with double sensitivity", "Change IGN on success"
    ]
  }
};
...
const BALANCE_SCENARIOS = {
  'ko_KR': [
    ...
    ["우리팀 라이너가 페이커(근데 던짐)", "적 팀 라이너가 페이커(근데 내가 이김)"],
    ["평생 핑 200에서 롤 하기", "평생 흑백 화면으로 롤 하기"],
    ["로또 1등 당첨 (100억)", "롤드컵 우승하고 명예의 전당 등극"],
    ["모든 판 60분 빡겜 승리", "모든 판 15분 서렌 패배"],
    ["내가 솔킬 따고 전챗으로 비웃기", "내가 솔킬 따이고 '역시 이상호' 듣기"]
  ],
  'en_US': [
    ...
    ["Ping 200 Forever", "Black & White Screen Forever"],
    ["Win Lottery ($10M)", "Win Worlds & Hall of Fame"]
  ]
};
...
function initPinball() {
  const canvas = document.getElementById('pinball-canvas'); if (!canvas) return;
  ...
  function setup() { canvas.width = canvas.parentElement.clientWidth; canvas.height = canvas.parentElement.clientHeight || 450; pins = []; spinners = []; const rows = 12, spacingX = canvas.width / 14, spacingY = 25; for (let r = 0; r < rows; r++) { const offset = (r % 2) * (spacingX / 2); for (let c = 0; c < 15; c++) { const x = c * spacingX + offset, y = 80 + r * spacingY; if (r > 2 && r < rows - 2 && c % 4 === 0 && Math.random() > 0.5) spinners.push({ x, y, angle: 0, speed: 0.05 + Math.random() * 0.05, length: 40 }); else pins.push({ x, y }); } } }
  startBtn.addEventListener('click', () => { if (isRunning) return; const opts = (input.value || "A,B,C,D").split(',').map(o => o.trim()).filter(o => o); if (!opts.length) return; focusTool('pinball-tool'); setup(); isRunning = true; balls = []; const tempBalls = []; opts.forEach((opt, i) => { for(let j=0; j<30; j++) tempBalls.push({ label: opt, color: `hsl(${(i * 360 / opts.length)}, 70%, 60%)` }); }); for (let i = tempBalls.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [tempBalls[i], tempBalls[j]] = [tempBalls[j], tempBalls[i]]; } balls = tempBalls.map((b, i) => ({ x: canvas.width / 2 + (Math.random() - 0.5) * 80, y: -20 - (i * 15), r: 8, label: b.label, color: b.color, vx: (Math.random() - 0.5) * 3, vy: 1, finished: false }));
    function loop() { ctx.clearRect(0, 0, canvas.width, canvas.height); ctx.fillStyle = '#c89b3c'; pins.forEach(p => { ctx.beginPath(); ctx.arc(p.x, p.y, 2, 0, Math.PI * 2); ctx.fill(); }); ctx.strokeStyle = '#00cfbc'; ctx.lineWidth = 3; spinners.forEach(s => { s.angle += s.speed; const x1 = s.x + Math.cos(s.angle) * 20, y1 = s.y + Math.sin(s.angle) * 20; const x2 = s.x - Math.cos(s.angle) * 20, y2 = s.y - Math.sin(s.angle) * 20; ctx.beginPath(); ctx.moveTo(x1, y1); ctx.lineTo(x2, y2); ctx.stroke(); }); let active = 0; balls.forEach(b => { if (b.finished) return; active++; b.vy += gravity; b.x += b.vx; b.y += b.vy; pins.forEach(p => { const dx = b.x - p.x, dy = b.y - p.y, dist = Math.sqrt(dx*dx + dy*dy); if (dist < b.r + 2) { const angle = Math.atan2(dy, dx), speed = Math.sqrt(b.vx*b.vx + b.vy*b.vy); b.vx = Math.cos(angle) * speed * bounciness + (Math.random()-0.5); b.vy = Math.sin(angle) * speed * bounciness; b.y += b.vy; } }); spinners.forEach(s => { const x1 = s.x + Math.cos(s.angle) * 20, y1 = s.y + Math.sin(s.angle) * 20, x2 = s.x - Math.cos(s.angle) * 20, y2 = s.y - Math.sin(s.angle) * 20, l2 = 40 * 40, t = Math.max(0, Math.min(1, ((b.x - x1) * (x2 - x1) + (b.y - y1) * (y2 - y1)) / l2)), closestX = x1 + t * (x2 - x1), closestY = y1 + t * (y2 - y1), dx = b.x - closestX, dy = b.y - closestY, dist = Math.sqrt(dx * dx + dy * dy); if (dist < b.r + 2) { const nx = dx / dist, ny = dy / dist, dot = b.vx * nx + b.vy * ny; b.vx = (b.vx - 2 * dot * nx) * bounciness + (Math.random() - 0.5) * 4; b.vy = (b.vy - 2 * dot * ny) * bounciness - 2; b.x = closestX + nx * (b.r + 3); b.y = closestY + ny * (b.r + 3); } }); if (b.x < b.r || b.x > canvas.width - b.r) { b.vx *= -0.7; b.x = b.x < b.r ? b.r : canvas.width - b.r; } if (b.y > canvas.height - 10) { b.finished = true; b.finishTime = Date.now(); } ctx.fillStyle = b.color; ctx.beginPath(); ctx.arc(b.x, b.y, b.r, 0, Math.PI * 2); ctx.fill(); ctx.fillStyle = "#fff"; ctx.font = "bold 9px Spiegel"; ctx.textAlign = "center"; ctx.fillText(b.label.substring(0, 3), b.x, b.y + 3); }); if (active > 0) requestAnimationFrame(loop); else { isRunning = false; const last = balls.reduce((p, c) => p.finishTime > c.finishTime ? p : c); showBigResult("THE FINAL WINNER", last.label); } }
    loop();
  });
...
function initLadder() {
  ...
  startBtn.addEventListener('click', () => { if (isRunning) return; const players = (pIn.value || "A,B,C,D,E").split(',').map(s => s.trim()).filter(s => s), results = (rIn.value || "1,2,3,4,5").split(',').map(s => s.trim()).filter(s => s), count = Math.min(players.length, results.length); if (count < 2) return; focusTool('ladder-tool'); isRunning = true; isSkipped = false; skipBtn.classList.remove('hidden'); const spacing = canvas.width / (count + 1), lines = []; for (let i = 0; i < count - 1; i++) { for (let j = 0; j < 15; j++) lines.push({ from: i, to: i + 1, y: 70 + Math.random() * 270 }); } const sortedLines = [...lines].sort((a,b) => a.y - b.y); let pathIdx = 0; function finish() { isRunning = false; skipBtn.classList.add('hidden'); const summary = players.map((p, i) => { let cx = i; sortedLines.forEach(l => { if (l.from === cx) cx = l.to; else if (l.to === cx) cx = l.from; }); return `${p}: ${results[cx]}`; }).join('\n'); showBigResult("LADDER RESULTS", summary); }
    function revealPath() { if (isSkipped) { drawAllPaths(); return; } ctx.clearRect(0, 0, canvas.width, canvas.height); drawStatic(players, results, count, spacing, lines, pathIdx); let currX = pathIdx, currY = 60, pathPoints = [{x: spacing * (currX + 1), y: currY}]; sortedLines.forEach(l => { if (l.from === currX) { pathPoints.push({x: spacing*(currX+1), y: l.y}); currX = l.to; pathPoints.push({x: spacing*(currX+1), y: l.y}); } else if (l.to === currX) { pathPoints.push({x: spacing*(currX+1), y: l.y}); currX = l.from; pathPoints.push({x: spacing*(currX+1), y: l.y}); } }); pathPoints.push({x: spacing * (currX + 1), y: 350}); let pointIdx = 0; function animateMarker() { if (isSkipped) return drawAllPaths(); if (pointIdx < pathPoints.length - 1) { const start = pathPoints[pointIdx], end = pathPoints[pointIdx+1]; let progress = 0; function drawMove() { if (isSkipped) return drawAllPaths(); ctx.clearRect(0, 0, canvas.width, canvas.height); drawStatic(players, results, count, spacing, lines, pathIdx); ctx.strokeStyle = `hsl(${pathIdx * 360 / count}, 80%, 50%)`; ctx.lineWidth = 5; ctx.beginPath(); ctx.moveTo(pathPoints[0].x, pathPoints[0].y); for(let i=0; i<=pointIdx; i++) ctx.lineTo(pathPoints[i].x, pathPoints[i].y); const midX = start.x + (end.x - start.x) * progress, midY = start.y + (end.y - start.y) * progress; ctx.lineTo(midX, midY); ctx.stroke(); ctx.fillStyle = "#fff"; ctx.beginPath(); ctx.arc(midX, midY, 6, 0, Math.PI*2); ctx.fill(); progress += 0.06; if (progress < 1) requestAnimationFrame(drawMove); else { pointIdx++; animateMarker(); } } drawMove(); } else { if (pathIdx < count - 1) { pathIdx++; setTimeout(revealPath, 1200); } else finish(); } } animateMarker(); }
...
function initTeamSplitter() {
  ...
  function createRow() { const row = document.createElement('div'); row.className = 'team-pair-row'; row.innerHTML = `<input type="text" class="pair-input" placeholder="BLUE"><button class="gap-btn">=</button><input type="text" class="pair-input" placeholder="RED"><button class="swap-btn">⇄</button>`; row.querySelector('.gap-btn').addEventListener('click', e => { const ops = ['=', '>', '<']; const curr = ops.indexOf(e.target.textContent); e.target.textContent = ops[(curr + 1) % ops.length]; }); row.querySelector('.swap-btn').addEventListener('click', () => { const ins = row.querySelectorAll('.pair-input'), tmp = ins[0].value; ins[0].value = ins[1].value; ins[1].value = tmp; const g = row.querySelector('.gap-btn'); if(g.textContent==='>') g.textContent='<'; else if(g.textContent==='<') g.textContent='>'; }); list.appendChild(row); }
...
function initMissionRoulette() {
  ...
  const resetAll = () => { populateStrip('roulette-champ', [], true); populateStrip('roulette-action', [], true); populateStrip('roulette-target', [], true); };
  resetAll();
  startBtn.addEventListener('click', async () => {
    if (startBtn.disabled) return; startBtn.disabled = true; resetAll();
...
function initBalanceGame() {
  ...
  const pick = () => {
    ...
    const scenarios = BALANCE_SCENARIOS[currentState.locale] || BALANCE_SCENARIOS['en_US'];
    let idx; do { idx = Math.floor(Math.random() * scenarios.length); } while (currentState.balanceHistory.includes(idx) && currentState.balanceHistory.length < scenarios.length);
    currentState.balanceHistory.push(idx); if (currentState.balanceHistory.length > 20) currentState.balanceHistory.shift();
    const s = scenarios[idx]; optA.querySelector('.option-text').textContent = s[0]; optB.querySelector('.option-text').textContent = s[1];
  };
